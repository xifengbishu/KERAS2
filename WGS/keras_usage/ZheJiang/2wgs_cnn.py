...
model.compile(loss='sparse_categorical_crossentropy', optimizer=sgd,class_mode="categorical")
...
#model.compile(loss='categorical_crossentropy',poisson,cosine_proximity,mape,msle,squared_hinge,hinge,cosine_proximity,kullback_leibler_divergence/kld
'''
sgd = SGD(lr=0.05, decay=1e-6, momentum=0.9, nesterov=True)
...
his=model.fit(data, label, batch_size=batch_size, nb_epoch=nb_epoch,shuffle=True,verbose=1,validation_split=0.1)

#early_stopping = EarlyStopping(monitor='val_loss', patience=2)
#his=model.fit(data, label, nb_epoch=nb_epoch, batch_size=batch_size,verbose=0)
...
for i in range(layers_num):
	...
	print ('layer_',i,'_output',layer_output.shape)
'''
...
# Save
###########
model.save_weights('wgs_cnn_weights.h5',overwrite=True)
exit()
###########
# Save
###########
